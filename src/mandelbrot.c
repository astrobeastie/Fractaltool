		...
		for (x = 0; x < width; x++) {
			...
			while (i < max_it
			  ...
			  //printf("(%d;%d) : ",(int)x,(int)y );
				//printf("Re: %.2g Im: %.2g n: %d\n",(double)c_re,(double)c_im, i);
			if (i < max_it)
	...
	for (y = 0; y < height; y++) {
		...
		for (x = 0; x < width; x++) {
			...
			while (i < max_it && d < BAILOUT) {
				...
				long double c_re_temp =
				    c_re * c_re - c_im * c_im + re;
				long double c_im_temp = 2 * c_re * c_im + im;
				long double d_re_temp =
				    2 * (c_re * d_re - c_im * d_im) + 1;
	...
	for(i=0;i<5;i++) {
		...
		for (y = 0l; y < height; y+=chunksize) {
			...
			if (eval==1) {
					re = xs + x / res;
					long double res = 0l;	
					_asm_fet(re, im,0,0, max_it, BAILOUT, &res);
